...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content-project">
		...
		<div class="column-right">
			<h2 class="entry-title-detail"><?php _e('DESCRIPTION','twentyeleven');?></h2>
			...
			<div class="showmenu">
			<a class="readmore-link" href="javascript:void(0)"><?php _e('READ FULL DESCRIPTION &gt;','twentyeleven');?></a>
			<?php 	if(!empty($custom_post['Googlemap-latitude'][0])&&!empty($custom_post['Googlemap-longitude'][0])&&!empty($custom_post['Googlemap-detail'][0])){?>
				<a  class="googlemap-link" href="#mapshow"><?php _e('VIEW LOCATION MAP &gt;','twentyeleven');?></a>
				<?php }?>
			</div>
	 ...
	 <div class="location"><?php _e('LOCATION: ','twentyeleven'); 
	 echo $custom_post['Location'][0];?></div>
	...
	<?php if(!empty($images)) :	?>
		<div class="carousel">
						...
						<?php for($i=0;$i<count($images);$i++){ 
							?>
							<a class="images-<?php echo $images[$i]->pid?>  <?php echo ($i==0)? 'active':'';?>" href="<?php echo home_url('wp-content/gallery/').$images[$i]->name.'/'.$images[$i]->filename;?>" rel="lightbox[<?php echo $post->ID?>]">
		 ...
		 <?php //print_p($custom_post);
			...
			foreach($custom_post['File Upload'] as $fid){
				if(!empty($fid)){
					...
					<a class="icon <?php echo showFileExtension($linkdownload);?>" href="<?php echo $linkdownload;?>" target="_blank"><?php echo (empty($custom_post['File name']))? showFileExtension($linkdownload):$custom_post['File name'][$fname_count] ;?> Download</a>
