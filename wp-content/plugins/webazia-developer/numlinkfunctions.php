...
function numlinksFrontpage($pagenum, $alldata,$postperpage, $pages_visible, $scriptname="", $get="") {
	if($postperpage>=$alldata) return;
	$maxpage=ceil($alldata/$postperpage);
	echo '<div class="paging">';
	if ($pagenum > 1) {
	//	echo '<td class="td-border"><a href="'.page_name(1, $scriptname, $get).'" class="numlinks">&laquo;</a></td>';   // first page
	 	echo '<a href="'.page_name(($pagenum-1), $scriptname, $get).'" class="numlinksFrontpage">&lt; Prev</a>';   // prev page
	} else {
		//echo '<td class="li-border numlinks-inactive">&laquo;</td>';   // first page
	 	echo '<span >&lt; Prev</span>';   // prev page
	}	
	...
	while ($i <= $pages_visible){
		if ($pagenum-ceil($pages_visible/2) < 0) {  //wenn unterer Bereich
			if ($i == $pagenum) echo '<span class="current">'.($pagenum).'</span>';
			else echo '<a href="'.page_name($i, $scriptname, $get).'">'.($i).'</a>';
			
		} else if ($pagenum+floor($pages_visible/2) > $maxpage) {  // wenn oberer bereich
			if($maxpage > $pages_visible) $j = $maxpage-$pages_visible+$i;
			else $j = $i;
			
			if ($j == $pagenum) echo '<span class="current">'.($pagenum).'</span>';
			else echo '<a href="'.page_name($j, $scriptname, $get).'">'.$j.'</a>';

			
		} else {  // wenn mittlerer bereich
		    if ($i == ceil($pages_visible/2)) echo '<span class="current">'.($pagenum).'</span>';
			else {
				$j = $pagenum-ceil($pages_visible/2)+$i;
				echo '<a href="'.page_name($j, $scriptname, $get).'">'.$j.'</a>';
			}
		}
		if ($i == $maxpage) break;
		$i++;
	...
	if ($pagenum < $maxpage){
		echo '<a href="'.page_name(($pagenum+1), $scriptname, $get).'">Next &gt;</a>';  // next page
		//echo '<a href="'.page_name($maxpage, $scriptname, $get).'" class="numlinks">&raquo;</a>';   // last page
	} else {
		echo '<span>Next &gt;</span>';   // next page
//	 	echo '<td class="li-border numlinks-inactive">&raquo;</td>';   // last page
	}	
